...
Effects of element size (plastic hinge elements length) in Baseline model (Model 0)
...
# Nodes on Right (Beam Hinge = 456 mm)
ops.node(10,  203.0, 0.0)                       # Right Rigid Link Node
ops.node(11,  203.0 + 456.0, 0.0)               # Plastic Hinge (Doubled)
ops.node(12,  659.0 + (848.0 - 659.0)/2 * 1.0, 0.0)
ops.node(13,  848.0, 0.0)                       # Right gravity load point P
ops.node(14,  848.0 + ((2119.0-848.0)/6)*1.0, 0.0)
ops.node(15,  848.0 + ((2119.0-848.0)/6)*2.0, 0.0)
ops.node(16,  848.0 + ((2119.0-848.0)/6)*3.0, 0.0)
ops.node(17,  848.0 + ((2119.0-848.0)/6)*4.0, 0.0)
ops.node(18,  848.0 + ((2119.0-848.0)/6)*5.0, 0.0)
ops.node(19,  2119.0, 0.0)                      # Right roller support

# Nodes on Left (Beam Hinge = 456 mm)
ops.node(30, -203.0, 0.0)                       # Left Rigid Link Node
ops.node(31, -203.0 - 456.0, 0.0)               # Plastic Hinge (Doubled)
ops.node(32, -659.0 - (848.0 - 659.0)/2 * 1.0, 0.0)
ops.node(33, -848.0, 0.0)                       # Left gravity load point P
ops.node(34, -848.0 - ((2119.0-848.0)/6)*1.0, 0.0)
ops.node(35, -848.0 - ((2119.0-848.0)/6)*2.0, 0.0)
ops.node(36, -848.0 - ((2119.0-848.0)/6)*3.0, 0.0)
ops.node(37, -848.0 - ((2119.0-848.0)/6)*4.0, 0.0)
ops.node(38, -848.0 - ((2119.0-848.0)/6)*5.0, 0.0)
ops.node(39, -2119.0, 0.0)                      # Left roller support

# Nodes on Top (Column Hinge = 406 mm)
ops.node(20, 0.0, 228.0)                        # Top joint face
ops.node(21, 0.0, 228.0 + 406.0)                # Plastic Hinge (Doubled)
ops.node(22, 0.0, 634.0 + ((1236.0-634.0)/4)*1.0)     
ops.node(23, 0.0, 634.0 + ((1236.0-634.0)/4)*2.0)     
ops.node(24, 0.0, 634.0 + ((1236.0-634.0)/4)*3.0)     
ops.node(25, 0.0, 1236.0)                       # Top tip loading node

# Nodes on Bottom (Column Hinge = 406 mm)
ops.node(40, 0.0, -228.0)                       # Bottom joint face
ops.node(41, 0.0, -228.0 - 406.0)               # Plastic Hinge (Doubled)
ops.node(42, 0.0, -634.0 - ((1236.0-634.0)/4)*1.0)     
ops.node(43, 0.0, -634.0 - ((1236.0-634.0)/4)*2.0)     
ops.node(44, 0.0, -634.0 - ((1236.0-634.0)/4)*3.0)     
...
output_folder = os.path.join("Final_Project", "Results", "Model_0_Doubled")
